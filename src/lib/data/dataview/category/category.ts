...
export class CategoryDataView extends DataView {
  ...
  constructor(dataSource: CARTOSource | Layer, column: string, options: CategoryDataViewOptions) {
  ...
  private async groupBy() {
    const sourceData = await this.getSourceData([this.column, this.operationColumn]);
