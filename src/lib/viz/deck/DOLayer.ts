...
export function getURLFromTemplate(template: string | string[], properties: any) {
...
export class DOLayer<T> extends MVTLayer<T> {
  ...
  getTileData(tile: ViewportTile) {
    ...
    return this.loadGeographiesAndData(tile, metadataURL).then(([geographies, data]) => {
      if (!data) {
  ...
  private loadGeographiesAndData(tile: ViewportTile, metadataURL: string) {
    return Promise.all([super.getTileData(tile), loadData(metadataURL, this.getLoadOptions())]);
  }
}
